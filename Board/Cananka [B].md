...
First we need to enable CAN driver:

    sudo bash -c 'echo "dtparam=spi=on" >> /boot/config.txt'
    sudo bash -c 'echo "dtoverlay=mcp2515-can0,oscillator=16000000,spimaxfrequency=8000000,interrupt=25" >> /boot/config.txt'
    sudo reboot

Then we can manually raise link using ip command (this example makes it work at 125 KHz):
...
For testing utilities install can-utils:

    sudo apt-get install can-utils

Then you can use cansend and candump, e.g.:
